...
foreach ($eqLogics as $eqLogic) {
	...
	if (($eqLogic->getConfiguration('batteryStatus') == '') && is_object($eqLogic->getCmd('info', 'Battery'))) {
		$battery = $eqLogic->getCmd('info', 'Battery')->execCmd();
    } else {
	...
	$idleMax = new DateTime("1 week ago");
	$tsIdleMax = $idleMax->getTimestamp();
	$lastCommunication = strtotime($eqLogic->getStatus("lastCommunication"));
	$dateFR = date("d/m/Y \à H\hi", $lastCommunication);
	$messageLevel = $lastCommunication < $tsIdleMax ? $label = "warning" : $label = "info";
	echo '<td><span class="label label-'.$label.'" style="font-size : 1em; cursor : default;">' . $eqLogic->getStatus('lastCommunication') . '</span></td>';
